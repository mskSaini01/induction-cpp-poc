...
String Logger::vFilePath;

String Logger::vFileName;

String Logger::vMasterFileName = "master_log.txt";
...
{
        String log_master_file  = vFilePath;
        
    // Write on severity files 
    Write (vFileName,       pSource);

    log_master_file.Append (MODE_PATH);
    log_master_file.Append (vMasterFileName);
...
void Logger::AddLog (eConfigurationMode pMode, eLogType pLogType, String pInfo, String pDetails)
{
    ...
    switch (pMode) {
        ...
        case DEBUG_MODE:
            vFileName.Append("\\Debug\\");
            break;
        case RELEASE_MODE:  
            vFileName.Append("\\Release\\");
            break;
    ...
    switch (pLogType) {
        ...
        case ERROR_TYPE:
            ...
            vFileName.Append ("error_");
            break;
        case WARNING_TYPE:
            log_entry.Append("[WARNING]  ");
            vFileName.Append ("warning_");
            break;
        case INFO_TYPE:
            log_entry.Append("[INFO]  ");
            vFileName.Append ("info_");
            break;
        default:
            break;
    }

    vFileName.Append (vMasterFileName);
...
void Logger::SetFileName (const char * pFileName)
{
    ...
    vMasterFileName = pFileName;
}

void Logger::SetFilePath ()
{
    // Set File Path Bin's Executables Project Folder
    vFilePath = (std::filesystem::current_path ().parent_path ().string ()).c_str ();

    vFilePath.Append ("\\Bin\\Executables\\");
    vFilePath.Append ((std::filesystem::current_path ().filename ().string ()).c_str ());
}
